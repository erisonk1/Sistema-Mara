...
export default function Ver() {
  ...
  const prevComandas = useRef<Comanda[]>([]);

  const fetchComandasHoje = async () => {
    const res = await fetch("http://localhost:4000/comandas/hoje");
  ...
  useEffect(() => {
    if (data.length > prevComandas.current.length) {
      const novas = data.filter(
        (c: Comanda) => !prevComandas.current.some((old) => old.id === c.id)
      );
      if (novas.length > 0) {
  ...
  const handleStatusChange = async (comandaId: number, novoStatus: Comanda["status"]) => {
    ...
    if (!comanda) return;
    try {
      await fetch(`http://localhost:4000/comandas/${comandaId}`, {
        method: "PUT",
  ...
  const salvarComanda = async (comanda: Comanda) => {
    ...
    try {
      const response = await fetch(`http://localhost:4000/comandas/${comanda.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
        ...
        {/* Comandas — mais recente primeiro */}
        <div className="flex flex-col-reverse gap-4">
          {comandas.map((comanda) => {
            const total = comanda.itens.reduce(
              (acc, i) => acc + (i.porPeso ? (i.peso ?? 0) : i.quantidade) * i.preco,
              0
            );
                    ...
                    <div className="space-y-3">
                      {comanda.itens.map((item, idx) => (
                        <div key={idx} className="grid grid-cols-1 sm:grid-cols-[1fr_auto_auto_auto] gap-2 items-center pb-2 border-b border-gray-50">
                          ...
                          {item.porPeso ? (
                            <input
                              type="number" step="0.01" value={item.peso ?? 0}
                          ...
                          <input
                            type="number" value={item.preco} step="0.01"
                            onChange={(e) => handleItemChange(comanda.id, idx, "preco", e.target.value)}
                            className="border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 w-24 focus:ring-2 focus:ring-rose-400 focus:outline-none"
                          />
                          <button
                            onClick={() => removeItem(comanda.id, idx)}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex rounded-xl overflow-hidden border border-gray-200">
                            {(["Pendente", "Concluído"] as const).map((s) => (
                              <button key={s}
                                onClick={() => handleStatusChange(comanda.id, s)}
